...
import {
  ...
  toLowerFirstCase,
  toPascalCase,
} from '../utils/code-generator.utils';
...
export class TemplateGeneratorService {
   ...
   */
  async generateCode(
    generator: CodeGenerator & { columns: CodeGeneratorColumn[] },
  ): Promise<GenerateCode[]> {
    const generatedFiles: GenerateCode[] = [];
    const options = this.parseOptions(generator.options);
    const modelName = tableNameToClassName(generator.tableName, generator.tablePrefix);

    // 生成模型文件
    generatedFiles.push(
      await this.renderTemplate('model.ejs', {
        generator,
        modelName,
        className: `${modelName}`,
    ...
    // 生成DTO文件
    generatedFiles.push(
      await this.renderTemplate('dto.ejs', {
        generator,
        modelName,
        className: `${modelName}`,
        camelName: toLowerFirstCase(modelName),
    ...
    // 生成Service文件
    generatedFiles.push(
      await this.renderTemplate('service.ejs', {
        generator,
        modelName,
        className: `${modelName}`,
        camelName: toLowerFirstCase(modelName),
    ...
    // 生成Controller文件
    generatedFiles.push(
      await this.renderTemplate('controller.ejs', {
        generator,
        modelName,
        className: `${modelName}`,
        camelName: toLowerFirstCase(modelName),
    ...
    // 生成Module文件
    generatedFiles.push(
      await this.renderTemplate('module.ejs', {
        generator,
        modelName,
        className: `${modelName}`,
        camelName: toLowerFirstCase(modelName),
    ...
    // 生成前端API文件
    generatedFiles.push(
      await this.renderTemplate('api.ejs', {
        generator,
        modelName,
        className: `${modelName}`,
        camelName: toLowerFirstCase(modelName),
    ...
    // 生成前端列表页面
    generatedFiles.push(
      await this.renderTemplate('list-page.ejs', {
        generator,
        modelName,
        className: `${modelName}`,
        camelName: toLowerFirstCase(modelName),
    ...
    // 生成前端表单组件
    generatedFiles.push(
      await this.renderTemplate('form-component.ejs', {
        generator,
        modelName,
        className: `${modelName}`,
        camelName: toLowerFirstCase(modelName),
