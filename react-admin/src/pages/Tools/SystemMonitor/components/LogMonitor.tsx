...
const LogMonitor: React.FC<LogMonitorProps> = ({ refreshInterval = 300000 }) => {
  ...
  if (error) {
    return <Alert type="error" message="获取日志统计数据失败" description={error} showIcon />;
  }
      ...
      <Card
        ...
        extra={
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            <Button type="primary" icon={<FileTextOutlined />} loading={analyzingLogs} onClick={handleAnalyzeLogs}>
              分析最新日志
            </Button>
            <RangePicker
              onChange={dates => {
            ...
            />
            <ReloadOutlined onClick={fetchData} spin={loading} style={{ cursor: 'pointer' }} />
          </div>
        }
      >
        {logAnalysis && (
          <Card title="最新日志分析结果" size="small" style={{ marginBottom: 24 }}>
            <Row gutter={[16, 16]}>
              ...
              <Col xs={24} sm={8} md={4}>
                <Statistic title="错误日志" value={logAnalysis.errorCount} valueStyle={{ color: logAnalysis.errorCount > 0 ? '#cf1322' : undefined }} />
              </Col>
              <Col xs={24} sm={8} md={4}>
        ...
        <Table dataSource={logStats} columns={columns} rowKey="id" pagination={{ pageSize: 10 }} size="middle" />
      </Card>
    </Spin>
  );
};
